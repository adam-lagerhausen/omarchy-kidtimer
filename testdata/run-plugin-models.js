...
assertEqual(kid.barLabel({ bedtime_active: false, groups: { fun: 59 } }), "1m left", "last minute chip")
assertEqual(kid.leftoverMinutes(59), 1, "last minute leftover")
assertEqual(kid.leftoverMinutes(0), 0, "zero leftover")
assertEqual(kid.leftoverMinutes(-3), 0, "negative leftover")
...
assertEqual(parent.projectFun({ funLeft: 59, spentFun: 0 }, 3600).leftLabel, "1m LEFT", "last minute parent")
assertEqual(parent.projectFun({ funLeft: 59, spentFun: 0 }, 3600).empty, false, "last minute parent not empty")
...
assertEqual(parent.notifyBody("Ada", { seconds: 1800 }, null, {}), "Ada asked for 30 more minutes", "notify body")
assertEqual(parent.notifyBody("Ada", { seconds: 600 }, null, { parentLocked: true }), "Ada asked to unlock", "notify body locked")
const ping = parent.pingCard({ id: "a1", seconds: 600 }, { id: "ada", name: "Ada", status: {} })
assertEqual(ping.head, "Ada", "ping head")
assertEqual(ping.body, "Ada asked for 10 more minutes", "ping body matches the panel card")
assertEqual(ping.text, "Ada asked for 10 more minutes", "ping text")
assertEqual(ping.kidId, "ada", "ping stores kid id")
assertEqual(parent.pingDecide(ping, "approve"), { id: "a1", kidId: "ada", decision: "approve" }, "ping approve")
assertEqual(parent.pingDecide(ping, "deny"), { id: "a1", kidId: "ada", decision: "deny" }, "ping deny")
assertEqual(parent.pingDecide(ping, "maybe"), null, "ping rejects junk")
assertEqual(parent.pingDecide(null, "approve"), null, "ping decide needs a card")
let flight = parent.enqueuePing([], ping)
let tap = parent.startPingDecide(flight, ping, "approve")
assertEqual(tap.act, { id: "a1", kidId: "ada", decision: "approve" }, "first ping tap")
assertEqual(tap.queue[0].busy, true, "ping in flight")
assertEqual(tap.queue.map(p => p.id), ["a1"], "in-flight tap keeps the ping")
assertEqual(parent.startPingDecide(tap.queue, ping, "deny"), null, "ignore ping tap while in flight")
assertEqual(parent.startPingDecide(tap.queue, ping, "approve"), null, "ignore repeat ping tap while in flight")
flight = parent.setPingBusy(tap.queue, ping, false)
assertEqual(flight[0].busy, false, "ping idle after request finishes")
assertEqual(parent.startPingDecide(flight, ping, "deny").act.decision, "deny", "ping tap after request finishes")
assertEqual(parent.startPingDecide(flight, ping, "maybe"), null, "in-flight helper still rejects junk")
assertEqual(parent.startPingDecide([], ping, "approve"), null, "no ping card no decide")
const pingTwo = parent.pingCard({ id: "a2", seconds: 1800 }, { id: "ada", name: "Ada" })
let two = parent.enqueuePing(parent.enqueuePing([], ping), pingTwo)
two = parent.startPingDecide(two, ping, "approve")
assertEqual(parent.startPingDecide(two.queue, pingTwo, "deny").act.id, "a2", "other ping still tappable")
assertEqual(parent.pingCard({ id: "a1", seconds: 600 }, { name: "Ada" }), null, "ping needs kid id")
let held = parent.enqueuePing([], ping)
assertEqual(parent.pingDecide(held[0], "approve").kidId, "ada", "tap uses stored kid id")
held = parent.prunePings(held, [{ id: "ada", claimed: false, status: {}, asks: [{ id: "a1", seconds: 600 }] }])
assertEqual(held.map(p => p.id), ["a1"], "non-200 poll keeps the ping")
held = parent.prunePings(held, [{ id: "ada", claimed: false, status: {}, asks: [] }])
assertEqual(held.length, 0, "successful poll prune drops the ping")
let pingQ = parent.enqueuePing([], ping)
pingQ = parent.enqueuePing(pingQ, ping)
assertEqual(pingQ.length, 1, "ping dedupe")
pingQ = parent.enqueuePing(pingQ, parent.pingCard({ id: "a2", seconds: 1800 }, { id: "ada", name: "Ada" }))
assertEqual(pingQ.length, 2, "second ping")
pingQ = parent.dismissPing(pingQ, ping)
assertEqual(pingQ.map(p => p.id), ["a2"], "dismiss ping")
assertEqual(parent.pingCard({ id: "a3", seconds: 600 }, { id: "ada", name: "Ada", status: { parentLocked: true } }).body,
  "Ada asked to unlock for 10 more minutes", "locked ping matches panel")
pingQ = parent.prunePings(pingQ, [{ id: "ada", name: "Ada", claimed: false, status: {}, asks: [{ id: "a2", seconds: 1800 }] }])
assertEqual(pingQ.map(p => p.id), ["a2"], "prune keeps pending")
const beaPing = parent.pingCard({ id: "bea-ask", seconds: 600 }, { id: "bea", name: "Bea", status: {} })
let moved = parent.enqueuePing([], beaPing)
moved = parent.prunePings(moved, [
  { id: "other", name: "Other", claimed: false, status: {}, asks: [] },
  { id: "bea", name: "Bea", claimed: false, status: {}, asks: [{ id: "bea-ask", seconds: 600 }] }
])
assertEqual(moved.map(p => p.id), ["bea-ask"], "prune keys by ask id not snapshot index")
moved = parent.prunePings(moved, [{ id: "bea", name: "Bea", claimed: false, status: {}, asks: [{ id: "bea-ask", seconds: 600 }] }])
assertEqual(moved.map(p => p.id), ["bea-ask"], "prune keeps ping after kid moves to index 0")
pingQ = parent.prunePings(pingQ, [{ name: "Ada", status: {}, asks: [] }])
assertEqual(pingQ.length, 0, "prune drops a decided ask")
assertEqual(parent.pingCard({ id: "a1", seconds: 600 }, { id: "ada", name: "<Ada>" }).head, "Ada", "ping strips host markup")
assertEqual(parent.snapshotById([{ id: "ada", name: "Ada" }, { id: "bea", name: "Bea" }], "bea").name, "Bea", "snapshot by id")
assertEqual(parent.snapshotById([{ id: "ada" }], "bea"), null, "missing snapshot")
assertEqual(parent.hour12ShouldPush({}, { id: "m1" }), true, "first clock push")
assertEqual(parent.hour12ShouldPush({ m1: true }, { id: "m1" }), false, "already pushed")
assertEqual(parent.hour12ShouldPush({}, { id: "m1", claimed: true }), false, "claimed no clock push")
assertEqual(parent.hour12ShouldPush({}, { url: "http://x" }), true, "clock push url key")
assertEqual(parent.hour12ShouldPush({}, {}), false, "no clock push key")
assertEqual(parent.hour12MarkPushed({}, { id: "m1" }).m1, true, "mark clock pushed")
assertEqual(parent.hour12MarkPushed({ m1: true }, { id: "m2" }).m1, true, "mark keeps prior")
...
assertEqual(kid.barLabel({
  ...
  bedtime_hold: true,
  groups: { fun: 1800 }
}), "30m left", "stay-up chip shows remaining")
assertEqual(kid.barUrgent({
  bedtime_active: true,
  bedtime_hold: true,
  groups: { fun: 1800 }
...
assertEqual(kid.clockFace({ groups: { fun: 59 } }).leftLabel, "1m LEFT", "last minute clock")
assertEqual(kid.clockFace({ groups: { fun: 59 } }).empty, false, "last minute clock not empty")
...
assertEqual(kid.headerSeconds({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }), 60, "save cover header seconds")
assertEqual(kid.headerSeconds({ overlay: true, groups: { fun: 0 }, save_seconds: 0 }), 0, "slam header seconds")
assertEqual(kid.barLabel({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }), "1m left", "save cover chip rounds up")
assertEqual(kid.barLabel({ overlay: true, groups: { fun: 0 }, save_seconds: 1 }), "1m left", "save cover last second chip")
assertEqual(kid.barLabel({ overlay: true, groups: { fun: 0 }, save_seconds: 0 }), "0m left", "slam chip")
assertEqual(kid.clockFace({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }).leftLabel, "1m LEFT", "save cover header rounds up")
assertEqual(kid.clockFace({ overlay: true, groups: { fun: 0 }, save_seconds: 1 }).leftLabel, "1m LEFT", "save cover last second header")
assertEqual(kid.clockFace({ overlay: true, groups: { fun: 0 }, save_seconds: 0 }).leftLabel, "0m LEFT", "slam header")
assertEqual(kid.clockFace({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }).empty, false, "save cover header is not empty")
assertEqual(kid.clockFace({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }).fill, 0, "save cover fill stays empty")
assertEqual(kid.barLabel({ overlay: true, bedtime_active: true, save_seconds: 60 }), "bedtime", "bedtime chip stays")
assertEqual(kid.clockFace({ overlay: true, bedtime_active: true, groups: { fun: 0 }, save_seconds: 60 }).leftLabel, "1m LEFT", "bedtime save header")
assertEqual(kid.clockFace({ overlay: true, parent_locked: true, groups: { fun: 0 }, save_seconds: 60 }).leftLabel, "0m LEFT", "lock skips save header")
...
assertEqual(kid.warningCopy("fun", 900), "15m left", "remaining copy")
...
assertEqual(warned.notices, ["15m left", "bedtime starts at 9:00 PM"], "15 min both")
warned = kid.takeWarnings(warned.state, mcStatus(300, 300))
assertEqual(warned.notices, ["5m left", "bedtime starts at 9:00 PM"], "5 min both")
warned = kid.takeWarnings(warned.state, mcStatus(60, 60))
assertEqual(warned.notices, ["1m left", "bedtime starts at 9:00 PM"], "1 min both")
...
assertEqual(warned.notices, ["15m left"], "warn again after grant")
...
assertEqual(kid.parseStatus({ overlay: true, save_seconds: 45 }).save_seconds, 45, "parse save seconds")
assertEqual(kid.saveSeconds({ save_seconds: 60 }), 60, "save seconds")
assertEqual(kid.saveSeconds({}), 0, "save seconds missing")
assertEqual(kid.overlayFace({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }), "save", "save cover empty")
assertEqual(kid.overlayFace({ overlay: true, bedtime_active: true, save_seconds: 12 }), "save", "save cover bedtime")
assertEqual(kid.overlayFace({ overlay: true, groups: { fun: 0 }, save_seconds: 0 }), "empty", "save done empty")
assertEqual(kid.overlayFace({ overlay: true, bedtime_active: true, save_seconds: 0 }), "bedtime", "save done bedtime")
assertEqual(kid.overlayFace({ overlay: true, parent_locked: true, save_seconds: 60 }), "locked", "lock skips save")
assertEqual(kid.overlaySaveCover({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }), true, "save cover on")
assertEqual(kid.overlaySaveCover({ overlay: true, groups: { fun: 0 }, save_seconds: 0 }), false, "save cover off")
assertEqual(kid.overlayBlocksSuper({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }), false, "super during save")
assertEqual(kid.overlayBlocksSuper({ overlay: true, groups: { fun: 0 }, save_seconds: 0 }), true, "super blocked at 0")
assertEqual(kid.overlayBlocksSuper({ overlay: true, parent_locked: true, save_seconds: 60 }), true, "lock blocks super")
assertEqual(kid.overlayWindowOn({ overlay: true, groups: { fun: 0 }, save_seconds: 60 }, false), true, "cover shown")
