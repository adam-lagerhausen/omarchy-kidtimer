...
assertEqual(selClaimed.chrome.adopt.index, 0, "claimed select asks share")
const sharePrompt = parent.adoptPrompt(claimedTape.kids[0])
assertEqual(sharePrompt.title, "Share TESTMAX?", "share title")
assertEqual(sharePrompt.body, "Another parent already claimed this computer. Yes lets this desk control it too.", "share body")
...
assertEqual(claimedTape.ours, false, "no does not take the box")
const adoptYes = parent.reduceChrome(selClaimed.chrome, { kind: "adoptYes" }, claimedTape)
assertEqual(adoptYes.adopt.index, 0, "yes shares")
