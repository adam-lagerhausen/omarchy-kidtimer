...
func TestPluginQMLIsHTTPClientNotBank(t *testing.T) {
	...
	mustContain(t, kidModel, `formatMinutes(seconds) + " left"`, "low-time notice matches chip")
	...
	mustContain(t, kidModel, "function leftoverMinutes", "last minute leftover")
	...
	mustContain(t, kidPanel, "if (status !== 200)", "failed ask keeps the picker")
	mustContain(t, kidPanel, "askFailLabel", "chip failed ask copy")
	mustContain(t, kidModel, "function askFailLabel", "ask fail copy")
	mustContain(t, kidModel, "try again", "ask fail line")
	...
	mustContain(t, kidPanel, "pinFailState", "cooldown is not wrong pin")
	...
	mustContain(t, kidModel, "function askStillWaiting", "waiting clears after the ask is gone")
	mustContain(t, kidModel, "function askWaitSeedPending", "post seeds pending so Waiting can clear")
	mustContain(t, kidPanel, "askStillWaiting", "chip uses ask wait helper")
	mustContain(t, kidPanel, "askWaitSeedPending", "chip seeds pending after Ask")
	...
	mustContain(t, kidOverlay, "askQueued = false", "failed overlay ask stays on Ask")
	mustContain(t, kidOverlay, "askStillWaiting", "overlay Waiting clears after the ask is gone")
	mustContain(t, kidOverlay, "askWaitSeedPending", "overlay seeds pending after Ask")
	mustContain(t, kidOverlay, "lastPending", "overlay tracks last pending count")
	mustContain(t, kidOverlay, "askFailLabel", "overlay failed ask copy")
	mustContain(t, kidOverlay, "overlayAskMinusOn", "overlay ask minus disables at the floor")
	mustContain(t, kidOverlay, "overlayAskPlusOn", "overlay ask plus disables at the ceil")
	mustContain(t, kidOverlay, "enabled: !root.waiting", "overlay Ask greys while sending")
	mustContain(t, kidOverlay, "pinBusy", "overlay Give time one in flight")
	...
	mustContain(t, kidModel, "function overlayPinFail", "overlay pin fail helper")
	mustContain(t, kidModel, "function pinFailState", "pin fail clears digits")
	mustContain(t, kidPanel, "pinBusy", "chip Parent Pin one in flight")
	mustContain(t, kidPanel, "pinFailState", "chip Parent Pin fail clears digits")
	mustContain(t, kidPanel, "function applyPinDigits", "chip pin boxes follow cleared digits")
	...
	mustContain(t, parentBar, "grantAllowed", "right-click grant respects bedtime, lock, and break")
	mustContain(t, parentModel, "function grantAllowed", "grant while bedtime, locked, or on a break")
	mustContain(t, parentModel, "snap.reachable === false", "no grant while the computer is off")
	mustContain(t, parentModel, "barLow: left <= 0", "time bar turns urgent only at zero")
	...
	mustContain(t, parentPanel, "t.kid.breaking", "tape greys +10 during a break")
	mustContain(t, parentPanel, "t.kid.away", "tape fades controls while the computer is off")
	mustContain(t, parentPanel, "snap.reachable === false", "settings do not save while the computer is off")
	mustContain(t, parentPanel, "ev.ask.away", "approve fades while that computer is off")
	mustContain(t, parentBar, "kid.reachable === false", "desk skips approve while off")
	...
	mustContain(t, parentPanel, "playMinusOn", "play minus fades at 15 minutes")
	mustContain(t, parentPanel, "breakPlusOn", "break plus fades at two hours")
	mustContain(t, parentPanel, "funDayMinusOn", "day minus fades at zero")
	mustContain(t, parentPanel, "t.kid.fun.empty) ? 1 : 0.35", "take ten fades when nothing is left")
	mustContain(t, parentModel, "function playMinusOn", "play floor helper")
	mustContain(t, parentModel, "function funDayPlusOn", "day ceil helper")
	mustContain(t, parentModel, "Screen time per day", "screen time header")
	mustContain(t, parentModel, "Break time", "break section title")
	mustContain(t, parentModel, "PLAY is how long they can sit. BREAK is how long they sit out.", "break section why")
	...
	mustContain(t, parentPanel, "pinSetEnabled", "tape pin set uses retry helper")
	...
	mustContain(t, parentModel, `"Computer"`, "missing app is computer")
	...
	mustContain(t, parentModel, "error: row.error === true", "household error reaches the tape")
	mustContain(t, parentBar, "error: row.error === true", "desk error caption")
	mustContain(t, parentModel, "function keepAsks", "offline keeps the last asks")
	mustContain(t, parentModel, "asksFetched", "household says when asks were fetched")
	mustContain(t, parentBar, "keepAsks", "desk apply keeps asks when offline")
	mustContain(t, parentModel, "function holdPolicy", "settings hold during a poll")
	mustContain(t, parentBar, "holdPolicy", "desk apply holds bedtime while saving")
	mustContain(t, parentModel, "function pinSetEnabled", "first pin can retry")
	mustContain(t, parentModel, "function pinRetryOn", "failed first pin retry")
	...
	mustContain(t, parentModel, "function leftoverMinutes", "parent last minute leftover")
	mustContain(t, parentModel, "function hour12ShouldPush", "clock push helper")
	mustContain(t, parentModel, "asked for ", "ask notify sentence")
